...
export default function JobDetail(){
    ...
    const handleApply = async () => {
        try { 
            const response = await axios.post('http://localhost:3068/api/applications', { job: id}, { 
                headers: {
    ...
    useEffect(() => {
        (async() => {
            try { 
                const response = await axios.get(`http://localhost:3068/api/jobs/${id}`)
    ...
    useEffect(() => {
        if(localStorage.getItem('token')) {
            (async() => {
                try { 
                    const response = await axios.get(`http://localhost:3068/api/applications/check/${id}`, { 
                        headers : {
                            Authorization: localStorage.getItem('token')
                        }
                    })
